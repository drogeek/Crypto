...
class DES:
    ...
    #   print("sbox="+format(sbox(i,(x&mask)>>(i*6)),'0x'))
        result+=DES.sbox(i,(x&mask)>>(i*6))<<(i*4)
        mask<<=6
        i+=1
      return result
    ...
    def __init__(self,m,k,compute_ki=CalculKi):
        self.K=k
        self.M=m
        self.Ki = compute_ki(self.K)

        #Application de la Permutation IP sur le message M 
        # le resultat est stocke dans tempo

        assert(all(map(lambda x : DES.verif_S[x]==DES.S(x),DES.verif_S)))
        tempo = 0
        for i,ip in enumerate(IP):
              tempo |= (((self.M >> (ip -1)) & 1) << i)

        #les 16 tours du DES 
        L = (tempo >> 32) 
        R = tempo & DES.MASK32 

        for subkey in self.Ki:
          Z = L 
          L = R 
...
if __name__ == "__main__":
    ...
    des = DES(m,k)
    print("Message = {}, Keys = {}, Crypto = {}".format(hex(des.M),list(map(format,des.Ki,repeat('0x'))),hex(des.C)))
